...
const Settings: React.FC = () => {
  ...
  // Auto-backup settings (persisted in localStorage)
  ...
  useEffect(() => {
    const loadLogs = async (): Promise<void> => {
      ...
      try {
        ...
        // Silently fail — logs are non-critical for the user
      } finally {
  ...
  // Auto-backup timer — periodically calls forceSync when enabled
  useEffect(() => {
    ...
    const intervalMs = intervalMap[autoBackupInterval] || 15 * 60 * 1000;
  ...
  /** Format login log action for display. */
  ...
  const statusConfig: Record<SyncStatus, { label: string; color: 'success' | 'error' | 'warning' | 'default' | 'info' }> = {
  ...
  return (
    <Box>
      <Typography
        variant="h5"
        sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1, mb: { xs: 2, sm: 3 } }}
      >
        ...
        {autoBackupEnabled && (
          <Box sx={{ mt: 1, ml: 1 }}>
            <FormControl size="small" sx={{ minWidth: 160, mb: 1 }}>
              ...
              >
                <MenuItem value="5">每 5 分钟</MenuItem>
                <MenuItem value="15">每 15 分钟</MenuItem>
                <MenuItem value="30">每 30 分钟</MenuItem>
                <MenuItem value="60">每 1 小时</MenuItem>
              </Select>
            </FormControl>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
              开启后，系统将定期执行额外同步备份，作为双重保障
            </Typography>
            <Typography variant="caption" color="text.secondary" display="block">
              上次备份：{lastBackupTime
                ? formatTime(lastBackupTime)
                : lastSyncTime
                  ? '尚未执行自动备份（数据已通过实时同步保存）'
                  : '从未同步'}
            </Typography>
            {nextBackupTime && (
              <Typography variant="caption" color="text.secondary" display="block">
                  ...
                  primary={
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                      <Typography variant="body2" component="span">
                        {`${formatAction(log.action)} · ${log.ip || '未知IP'}`}
                      </Typography>
                      {log.device && (
                        <Chip label={log.device} size="small" variant="outlined" />
                      )}
                    </Box>
                  }
      ...
      <Dialog open={recoverDialogOpen} onClose={() => setRecoverDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            这将清空本地所有数据并从服务器重新下载。本地未同步的更改将丢失。确定继续吗？
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setRecoverDialogOpen(false)}>取消</Button>
          <Button color="warning" variant="contained" onClick={handleFullRecover}>
            确认恢复
          </Button>
        </DialogActions>
      </Dialog>
